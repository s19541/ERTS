...
namespace ErtsApiFetcher.RecurringJobs {
    [RecurringJobInfo(typeof(WeeklyRecurringJob), nameof(WeeklyRecurringJob), ErtsCron.Never)]
    public class WeeklyRecurringJob : RecurringJobBase {
        private readonly DateTime from = DateTime.Now.AddDays(-7);

        public WeeklyRecurringJob(ErtsContext context, AppConfig appConfig, ApiDataProcessorExecutor executor) : base(context, appConfig, executor) {
        }
